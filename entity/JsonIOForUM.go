...
func UReadFromJsonFile(userFile string) []User {
	...
	var check Check = mystring{"UReadFromJsonFile"}; check.CheckError(err)
...
func UWriteToJsonFile(users []User, userFile string) {
	...
	var check Check = mystring{"UWriteToJsonFile"}; check.CheckError(err)
	for num := range users {
...
func MReadFromJsonFile(meetingFile string) []Meeting {
	...
	var check Check = mystring{"MReadFromJsonFile"}; check.CheckError(err)
	defer inputFile.Close()
...
func MeetingJsonEncode(meeting Meeting) []byte {
	...
	var check Check = mystring{"MeetingJsonEncode"}; check.CheckError(err)
	return meetingByte
}
...
func MWriteToJsonFile(meetings []Meeting, meetingFile string) {
	...
	var check Check = mystring{"MWriteToJsonFile"}; check.CheckError(err)
